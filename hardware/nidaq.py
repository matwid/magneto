...
class CounterBoard:
    ...
    def __init__(self, CounterIn, CounterOut, TickSource, SettlingTime=2e-3, CountTime=8e-3):
        ...
        print( "Max counts", self._MaxCounts)
        print("multiplied", self._MaxCounts*DutyCycle/f)
        ...
        #CHK(  dll.DAQmxSetCIPulseWidthTerm( self.CITask, self._CIDevice, '/dev1/PFI0' )  )
...
class AnalogOutBurst():
    ...
    def configure(self, n_samples, seconds_per_point):
        ...
        Configures the sampling length and rate.
            ...
            0<n<Inf: single waveform
        """
        if n_samples == self.n_samples and seconds_per_point == self.seconds_per_point:
            return
        if n_samples == 0:
            CHK( dll.DAQmxSetSampTimingType(self.ao_task, DAQmx_Val_OnDemand) )
        elif n_samples < numpy.inf:
    ...
    def line(self, voltage, seconds_per_point):
        ...
        CHK( dll.DAQmxWaitUntilTaskDone(ao_task, ctypes.c_double(timeout)) )
...
class DAQmxTrigger(object):
    def __init__(self, task, trigger_source, trigger_edge ):
        print("Configuring trigger for source port" , trigger_source)
        if trigger_edge == 'rising':
            ...
            return
        CHK( dll.DAQmxCfgDigEdgeStartTrig(task, trigger_source, edge) )


def DAQmxGetReadAvailSampPerChan(task):
    available = ctypes.c_uint32(-1)
    CHK( dll.DAQmxGetReadAvailSampPerChan(task, ctypes.byref(available)) )
    return available.value

def DAQmxGetSampClkRate(task):
    rate = ctypes.c_double(-1.)
    CHK( dll.DAQmxGetSampClkRate( task, ctypes.byref(rate)) )
    return rate.value

class AITask(object):
    """Analog input N values with frequency f"""
    def __init__(self, Channels, N, f, read_timeout=1.0, ranges=[(-10, 10)], task=None, continuous=False, clock_source='OnboardClock'):
        if not type(Channels) == list:
            raise TypeError("Channels must be given as list, e.g. ['dev1/ai0',...]")
        if  not len(Channels) == len(ranges):
            raise ValueError("Length of Channels and ranges must be equal")
        self.Channels = Channels
        self.N = N
        self.f = f
        ...
        self.data    = numpy.zeros((len(self.Channels),N), dtype=numpy.double )
        if task == None:
            self.__task_created = True
            self.Task = ctypes.c_ulong()
            CHK( dll.DAQmxCreateTask('', ctypes.byref(self.Task)) )
        else:
            self.__task_created = False
            self.Task = task

        if continuous:
            sampling = DAQmx_Val_ContSamps
        else:
            sampling = DAQmx_Val_FiniteSamps

        for i, channel in enumerate(self.Channels):
            CHK( dll.DAQmxCreateAIVoltageChan(self.Task, channel, '', DAQmx_Val_RSE, ctypes.c_double(self.ranges[i][0]), ctypes.c_double(self.ranges[i][1]), DAQmx_Val_Volts, ''))
        CHK( dll.DAQmxCfgSampClkTiming(self.Task, self.clock_source, ctypes.c_double(f), DAQmx_Val_Rising, sampling, ctypes.c_ulonglong(N)) )
    ...
    def Read(self):
        CHK( dll.DAQmxReadAnalogF64(self.Task,
                                    ...
                                    ctypes.c_ulong(self.N*len(self.Channels)),
...
class DITask(object):
    """Digital input N values with frequency f"""
    def __init__(self, Lines, N, f, read_timeout=1.0, task=None, continuous=False, clock_source='OnboardClock'):
        if not type(Lines) == list:
            raise TypeError("Lines must be given as list, e.g. ['dev1/port0/line16',...]")
        self.Lines = Lines
        self.N = N
        self.f = f
        self.clock_source = clock_source
        ...
        self.timeout = ctypes.c_double(4.*N/f)
        self.Nread = ctypes.c_uint32()
        self.bps = ctypes.c_uint32() #bytes per sample
        self.data    = numpy.zeros((len(self.Lines),N), dtype=numpy.bool )
        if task == None:
            self.__task_created = True
            self.Task = ctypes.c_ulong()
            CHK( dll.DAQmxCreateTask('', ctypes.byref(self.Task)) )
        ...
        for line in self.Lines:
            CHK( dll.DAQmxCreateDIChan(self.Task, line, '', DAQmx_Val_ChanPerLine))
        CHK( dll.DAQmxCfgSampClkTiming(self.Task, self.clock_source, ctypes.c_double(f), DAQmx_Val_Rising, sampling, ctypes.c_ulonglong(N)) )

    def Read(self):
        # CHK( dll.DAQmxReadDigitalLines(self.Task,
        #                             ctypes.c_long(self.N),
        #                             ctypes.c_double(self.read_timeout),
        #                             DAQmx_Val_GroupByChannel,
        #                             self.data.ctypes.data_as(c_float64_p),
        ...
        CHK(
            dll.DAQmxReadDigitalLines(
                    self.Task,                              #taskHandle  TaskHandle  The task to read samples from.
                    ctypes.c_int32(self.N),                 #numSampsPerChan     int32   The number of samples, per channel, to read. The default value of -1 (DAQmx_Val_Auto) reads all available samples. If readArray does not contain enough space, this function returns as many samples as fit in readArray.
                    ...
                    ctypes.c_double(self.read_timeout),     #timeout     float64     The amount of time, in seconds, to wait for the function to read the sample(s). To specify an infinite wait, pass -1 (DAQmx_Val_WaitInfinitely). This function returns an error if the timeout elapses.
                    ...
                    DAQmx_Val_GroupByChannel,               #fillMode    bool32  Specifies whether or not the samples are interleaved.
                                                            #Value       Description
                                                            #DAQmx_Val_GroupByChannel        Group by channel (non-interleaved)
                                                            #DAQmx_Val_GroupByScanNumber         Group by scan number (interleaved)
                    self.data.ctypes.data_as(c_uint8_p),
                    ctypes.c_uint32(self.N*len(self.Lines)),                #arraySizeInBytes    uInt32  The size of the array, in samples, into which samples are read.
                                                            #Output
                    ...
                    ctypes.byref(self.Nread),                 #sampsPerChanRead    int32 *     The actual number of samples read from each channel.
                    ctypes.byref(self.bps),                                   #numBytesPerSamp     int32 *     The number of elements in readArray that constitutes a sample per channel. For each sample per channel, numBytesPerSamp is the number of bytes that channel consists of.
                    None, #reserved
